...
class Pharmacist extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('pharmacist_model');
         ...
         if (!$this->ion_auth->in_group(array('admin')) && !in_array("Pharmacist", $permissions_check) &&  !in_array("Human Resource", $permissions_check)) {
        ...
        if ($this->form_validation->run() == FALSE) {
            if (!empty($id)) {
                ...
                $data['pharmacist'] = $this->pharmacist_model->getPharmacistById($id);
                 $data['permissions'] = $this->db->get('permission_features')->result();
                 $data['pharmacist_permissions'] = $this->pharmacist_model->getPharmacistByIonUserIdFromUsers($data['pharmacist']->ion_user_id);
                $this->load->view('home/dashboard'); // just the header file
                $this->load->view('add_new', $data);
            ...
            if (empty($id)) {     // Adding New Pharmacist
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $this->ion_auth->updateSerialId('pharmacist', $ion_user_id,$this->hospital_id);
                ...
                $this->pharmacist_model->updateIonUserId($username, $email, $password, $phone, $ion_user_id, $permission);
    ...
    function editPharmacistByJason() {
        ...
        $pharmacist_permission = $this->pharmacist_model->getPharmacistByIonUserIdFromUsers($data['pharmacist']->ion_user_id)->permissions;
        $permissions = explode(',', $pharmacist_permission);
        $option = '<label for="exampleInputEmail1">' . lang('module_permission') . ' </label><br>';
        foreach ($all_permissions as $all) {
            if ( $all->feature == 'Prescription' || $all->feature == 'Medicine' || $all->feature == 'Email' || $all->feature == 'Pharmacy') {
