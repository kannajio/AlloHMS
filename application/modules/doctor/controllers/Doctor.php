...
class Doctor extends MX_Controller {
    ...
    public function addNew() {
        ...
        if ($this->form_validation->run() == FALSE) {
            if (!empty($id)) {
                ...
                $data['permissions'] = $this->db->get('permission_features')->result();
                $data['doctors_permissions'] = $this->doctor_model->getDoctorByIonUserIdFromUsers($data['doctor']->ion_user_id);
                $this->load->view('home/dashboard'); // just the header file
                $this->load->view('add_new', $data);
            ...
            if (empty($id)) {     // Adding New Doctor
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $this->ion_auth->updateSerialId('doctor', $ion_user_id,$this->hospital_id);
    ...
    function editDoctorByJason() {
        ...
        $doctor_permission = $this->doctor_model->getDoctorByIonUserIdFromUsers($data['doctor']->ion_user_id)->permissions;
        $permissions = explode(',', $doctor_permission);
        $option = '<label for="exampleInputEmail1">' . lang('module_permission') . ' </label><br>';
        foreach ($all_permissions as $all) {
             if ($all->feature == 'Dashboard' || $all->feature == 'Patient' || $all->feature == 'Appointment' || $all->feature == 'Patient Encounter' || $all->feature == 'Prescription'|| $all->feature == 'Report' || $all->feature == 'Email'|| $all->feature == 'Lab') {
