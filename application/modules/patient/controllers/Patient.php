...
//echo "<pre>"; print_r($_POST); exit;
    ...
    function editPatient() {
        ...
        // $data['permissions'] = $this->db->get('permission_features')->result();
     //   $data['patients_permissions'] = $this->patient_model->getPatientByIonUserIdFromUsers($data['patient']->ion_user_id);
        $data['doctors'] = $this->doctor_model->getDoctor();
        $data['groups'] = $this->donor_model->getBloodBank();
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('add_new', $data);
    ...
    function editPatientByJason() {
        ...
        $patient_permission = $this->patient_model->getPatientByIonUserIdFromUsers($data['patient']->ion_user_id)->permissions;
        $permissions = explode(',', $patient_permission);
...
// Validating Deposited Amount Field
    ...
    function addMedicalHistory() {
        ...
        $this->form_validation->set_rules('description', 'Description', 'trim|max_length[10000]|xss_clean');
    ...
    function medicalHistory() {
        ...
        $data['checkups'] = $this->checkup_model->getCheckupsByPatient($data['patient']->id);
        //echo "<pre>"; print_r($data['checkups']); exit;
        $data['patients'] = $this->patient_model->getPatient();
        $data['doctors'] = $this->doctor_model->getDoctor();
    ...
    function getPatient($campid = '',$type = '') {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['patients'] = $this->patient_model->getPatientBysearch($search,$campid,$type);
            } else {
                $data['patients'] = $this->patient_model->getPatient($campid,$type);
            }
        } else {
            if (!empty($search)) {
                $data['patients'] = $this->patient_model->getPatientByLimitBySearch($limit, $start, $search, $campid, $type);
            } else {
                $data['patients'] = $this->patient_model->getPatientByLimit($limit, $start, $campid, $type);
            }
        }
        ...
        foreach ($data['patients'] as $patient) {
            ...
            $options3 = '<a class="btn green" target="_blank" title="' . lang('history') . '" style="color: #fff;" href="patient/medicalHistory?id=' . $patient->id . '"><i class="fa fa-stethoscope"></i> ' . lang('history') . '</a>';

            $options4 = '<a class="btn invoicebutton" target="_blank" title="' . lang('payment') . '" style="color: #fff;" href="finance/patientPaymentHistory?patient=' . $patient->id . '"><i class="fa fa-money-bill-alt"></i> ' . lang('payment') . '</a>';
            ...
            if ($this->ion_auth->in_group(array('admin'))) {
                $info[] = array(
                    ...
                    $patient->name,
                    $patient->phone,
                    '<a href="camp/patientList/'.$patient->camp_id.'">'.$patient->camp_name.'</a>',
        ...
        if (!empty($data['patients'])) {
            $output = array(
                ...
                "recordsTotal" => count($data['patients']),
...
// Get users
    ...
    public function viewCheckup() {
        ...
        //echo $id; exit;
        if($id) {
            $data['temp'] = $this->patient_model->getformTempById($id);
            $data['checkupTags'] = $this->patient_model->getCheckupTagsById($id);
            $data['fields'] = $this->patient_model->getformFieldsById($data['temp']->tid,$id);
        }
