...
class Settings extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('formtemplate_model');
         ...
         if (!$this->ion_auth->in_group(array('admin', 'superadmin')) && !in_array("Settings", $permissions_check)) {
    ...
    public function update() {
        ...
        if (!empty($email)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array();
                if(!empty($datalogo)) {
                    $this->settings_model->updateLogo($datalogo);
                }
                if($error) {
    ...
    function backup_database() {
        $permissions_check = $this->settings_model->modules(); 
        if (!$this->ion_auth->in_group(array('admin')) && !in_array("Settings", $permissions_check)) {
    ...
    function backup_files() {
        $permissions_check = $this->settings_model->modules();  if ($this->ion_auth->in_group(array('admin')) && !in_array("Settings", $permissions_check)) {
            $this->session->set_flashdata('error', lang('access_denied'));
            redirect("home/permission");
        }
...
// Errors";
    ...
    public function formtemplate() {
        $data['templates'] = $this->formtemplate_model->getTemplates();
    ...
    public function add_new($id = '') {
        ...
        if($this->input->post()) {
            if($_POST['id']) {
                ...
                $this->formtemplate_model->updateTemplate($id,$name);
                $this->formtemplate_model->deleteTemplateFields($id);
                $i=0;
                foreach($_POST['vitals'] as $val) {
                    ...
                    if($val) {
                        ...
                        $insert['field_id'] = $val;
                        $insert['field_type'] = $_POST['field_type'][$i];
                        $tf = $this->formtemplate_model->insertTemplate('template_fields',$insert);
                    }
                    $i++;
                }
                $this->session->set_flashdata('feedback', 'Template Updated Successfully');
                redirect('settings/formtemplate');
            } else {
                //echo "<pre>"; print_r($_POST); exit;
                $name['name'] = $_POST['name'];
                $temp_id = $this->formtemplate_model->insertTemplate('form_templates',$name);
                $i=0;
                foreach($_POST['vitals'] as $val) {
                    $insert = array();
        ...
        if($id) {
            $data['temp'] = $this->formtemplate_model->getformTempById($id);
            $data['fields'] = $this->formtemplate_model->getformFieldsById($id);
        }
        //echo "<pre>"; print_r($data); exit;
        $data['vitals'] = $this->formtemplate_model->getVitalandSymptoms();
        $this->load->view('home/dashboard'); // just the header file
    ...
    public function getvitals() {
        ...
        $vitals = $this->formtemplate_model->getVitalandSymptoms($searchTerm);
        $data = array();
    ...
    public function getaddfields() {
        $data = $this->formtemplate_model->getVitalandSymptoms();
        
        $html = '';
            $html .= '<div style="height:53px; clear:both;" class="productdiv" id="'.$_POST['length'].'"><div class="col-xs-6 col-md-6">
            <select name="vitals[]" class="form-control" ><option value="">--Select--</option>';
            foreach($data as $val) {
                    $html .= '<option value="'.$val->id.'">'.$val->name.'</option>';
            } 
            $html .= '</select>';
            $html .= '</div><div class="col-xs-5 col-md-5">
            <select name="field_type[]" class="form-control m-bot15" required>
    ...
    public function form_delete() {
        $id = $_GET['id'];
        $this->formtemplate_model->deleteFormtemplate($id);
