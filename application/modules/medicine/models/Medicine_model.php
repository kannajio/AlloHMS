...
class Medicine_model extends CI_model {
    ...
    function getExpiryMedicine() {
        $NewDate=date('Y-m-d', strtotime('+3 days'));
        $curDate=date('Y-m-d');
        $where = "(date(e_date) >= date('".$curDate."') AND date(e_date) <= date('".$NewDate."'))";
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->where($where);
        $this->db->order_by('id', 'asc');
        $query = $this->db->get('medicine');
    ...
    function getExpiryMedicineByPageNumber($page_number) {
        $NewDate=date('Y-m-d', strtotime('+3 days'));
        $curDate=date('Y-m-d');
        $where = "(date(e_date) >= date('".$curDate."') AND date(e_date) <= date('".$NewDate."'))";
        $data_range_1 = 50 * $page_number;
    ...
    function getExpiryMedicineBySearch($search) {
        $NewDate=date('Y-m-d', strtotime('+3 days'));
        $curDate=date('Y-m-d');
        $where = "(date(e_date) >= date('".$curDate."') AND date(e_date) <= date('".$NewDate."'))";
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    ...
    function getExpiryMedicineByLimitBySearch($limit, $start, $search) {
        $NewDate=date('Y-m-d', strtotime('+3 days'));
        $curDate=date('Y-m-d');
        $where = "(date(e_date) >= date('".$curDate."') AND date(e_date) <= date('".$NewDate."'))";
        $this->db->where($where);
    ...
    function getExpiryMedicineByLimit($limit, $start) {
        $NewDate=date('Y-m-d', strtotime('+3 days'));
        $curDate=date('Y-m-d');
        $where = "(date(e_date) >= date('".$curDate."') AND date(e_date) <= date('".$NewDate."'))";
        $this->db->where($where);
