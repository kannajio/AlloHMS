...
class Laboratorist extends MX_Controller {
    ...
    function __construct() {
       ...
       $permissions_check = $this->settings_model->modules();
        if (!$this->ion_auth->in_group(array('admin')) && !in_array("Laboratorist", $permissions_check) &&  !in_array("Human Resource", $permissions_check)) {
    ...
    public function addNew() {
        ...
        if ($this->form_validation->run() == FALSE) {
            if (!empty($id)) {
                ...
                $data['permissions'] = $this->db->get('permission_features')->result();
                $data['laboratorists_permissions'] = $this->laboratorist_model->getLaboratoristByIonUserIdFromUsers($data['laboratorist']->ion_user_id);
                $this->load->view('home/dashboard'); // just the header file
                $this->load->view('add_new', $data);
            ...
            if (empty($id)) {     // Adding New laboratorist
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $this->ion_auth->updateSerialId('laboratorist', $ion_user_id,$this->hospital_id);
                ...
                $this->laboratorist_model->updateIonUserId($username, $email, $password, $phone, $ion_user_id, $permission);
    ...
    function editLaboratoristByJason() {
        ...
        $laboratorists_permission = $this->laboratorist_model->getLaboratoristByIonUserIdFromUsers($data['laboratorist']->ion_user_id)->permissions;
       $permissions = explode(',', $laboratorists_permission);
       $option='<label for="exampleInputEmail1">'. lang('module_permission') .' </label><br>';
       foreach ($all_permissions as $all){
                if ( $all->feature == 'Patient' || $all->feature == 'Lab' || $all->feature == 'Report' || $all->feature == 'Email' || $all->feature == 'Donor') { 
