...
class Pharmacy extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin')) && !in_array("Pharmacy", $permissions_check)) {
    ...
    function editPurchaseByJason() {
        $id = $this->input->get('id');
        $data['purchase'] = $this->pharmacy_model->getPurchaseById($id);
        echo json_encode($data);
    }
    ...
    function getPurchaseList() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['purchases'] = $this->pharmacy_model->getPurchaseBysearch($search);
            } else {
                $data['purchases'] = $this->pharmacy_model->getPurchase1();
            }
        } else {
            
            if (!empty($search)) {
                $data['purchases'] = $this->pharmacy_model->getPurchaseByLimitBySearch($limit, $start, $search);
            } else {
                $data['purchases'] = $this->pharmacy_model->getPurchaseByLimit($limit, $start);
            }
        }
        // echo "<pre>";
        // print_r($data);
        ...
        foreach ($data['purchases'] as $purchase) {
            ...
            $option1 = '<button type="button" class="btn btn-info btn-xs btn_width editbutton" data-toggle="modal" data-id="' . $purchase->id . '"><i class="fa fa-edit"> </i> ' . lang('edit') . '</button>';

            $option2 = '<a class="btn btn-info btn-xs btn_width delete_button" href="finance/pharmacy/delete_purchase?id=' . $purchase->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"> </i> ' . lang('delete') . '</a>';
            $info[] = array(
                $i,
                $purchase->product,
                $purchase->seller,
        ...
        if (!empty($data['purchases'])) {
            $output = array(
                "draw" => intval($requestData['draw']),
                "recordsTotal" => $this->db->get('purchase')->num_rows(),
                "recordsFiltered" => $this->db->get('purchase')->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
    ...
    function delete_purchase() {
        ...
        if($_GET['id']) {
            $this->pharmacy_model->deletePurchase($_GET['id']);
            $this->session->set_flashdata('feedback', lang('deleted'));
            redirect('finance/pharmacy/purchase');
        }
    }
