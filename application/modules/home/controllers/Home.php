...
class Home extends MX_Controller {
    ...
    public function index() {
        if (!$this->ion_auth->in_group(array('superadmin'))) {
            ...
            if ($this->ion_auth->in_group(array('Human Resource'))) {
    ...
    public function updateids() {
		...
		->get('hospital');
		$user = $query->row();
        $hos = strtoupper(substr($user->name,0,2));
        $updateDoctor = $this->home_model->updateExistUser($hos,'doctor');
        $updatePatient = $this->home_model->updateExistUser($hos,'patient');
        $updateNurse = $this->home_model->updateExistUser($hos,'nurse');
        $updateAccountant = $this->home_model->updateExistUser($hos,'accountant');
        $updateLaboratorist = $this->home_model->updateExistUser($hos,'laboratorist');
        $updatePharmacist = $this->home_model->updateExistUser($hos,'pharmacist');
        $updateReceptionist = $this->home_model->updateExistUser($hos,'receptionist');
        $updateHr = $this->home_model->updateExistUser($hos,'human_resource');
        $updatePayroll = $this->home_model->updateExistUser($hos,'payroll');
