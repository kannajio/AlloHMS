...
class Nurse extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin')) && !in_array("Nurse", $permissions_check) &&  !in_array("Human Resource", $permissions_check)) {
    ...
    public function addNew() {
        ...
        if ($this->form_validation->run() == FALSE) {
            if (!empty($id)) {
                ...
                $data['permissions'] = $this->db->get('permission_features')->result();
                $data['nurses_permissions'] = $this->nurse_model->getNurseByIonUserIdFromUsers($data['nurse']->ion_user_id);
                $this->load->view('home/dashboard'); // just the header file
                $this->load->view('add_new', $data);
            ...
            if (empty($id)) {     // Adding New Nurse
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $this->ion_auth->updateSerialId('nurse', $ion_user_id,$this->hospital_id);
                ...
                $this->nurse_model->updateIonUserId($username, $email, $password, $phone, $ion_user_id, $permission);
    ...
    function editNurseByJason() {
        ...
        $nurse_permission = $this->nurse_model->getNurseByIonUserIdFromUsers($data['nurse']->ion_user_id)->permissions;
        $permissions = explode(',', $nurse_permission);
        $option = '<label for="exampleInputEmail1">' . lang('module_permission') . ' </label><br>';
        foreach ($all_permissions as $all) {
            if ($all->feature == 'Patient' || $all->feature == 'Appointment' || $all->feature == 'Patient Encounter' || $all->feature == 'Bed'|| $all->feature == 'Report' || $all->feature == 'Email'|| $all->feature == 'Donor') {
