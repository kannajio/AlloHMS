...
class Appointment extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Patient','Nurse')) && !in_array("Appointment", $permissions_check)) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($patient == 'add_new') {
                ...
                if ($this->ion_auth->email_check($p_email)) {
                    ...
                    $dfg = 5;
                    $this->ion_auth->register($username, $password, $p_email, $p_phone, $dfg);
    ...
    function getAppoinmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($this->ion_auth->in_group(array('Doctor','admin','Nurse'))) {
                if ($appointment->status == 'Confirmed') {
                    ...
                    else if ($appointment->live == '1') {
                        $options7 = '<a class="btn btn-info btn-xs btn_width livebutton" title="' . lang('start_live') . '" style="color: #fff;" href="meeting/instantLive?id=' . $appointment->id . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this patient? SMS and Email will be sent to the Patient.\');"><i class="fa fa-headphones"></i> ' . lang('live') . '</a>';
                    } else {
                        $options7 = '';
                    }
                } else {
