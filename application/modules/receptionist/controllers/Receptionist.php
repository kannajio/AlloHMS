...
class Receptionist extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin')) && !in_array("Receptionist", $permissions_check) &&  !in_array("Human Resource", $permissions_check)) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {     // Adding New Receptionist
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $this->ion_auth->updateSerialId('receptionist', $ion_user_id,$this->hospital_id);
                ...
                $this->receptionist_model->updateIonUserId($username, $email, $password, $phone, $ion_user_id, $permission);
    ...
    function editReceptionist() {
        ...
        $data['permissions'] = $this->db->get('permission_features')->result();
        $data['receptionist_permission'] = $this->receptionist_model->getReceptionistByIonUserIdFromUsers($data['receptionist']->ion_user_id);
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('add_new', $data);
    ...
    function editReceptionistByJason() {
        ...
        $receptionist_permission = $this->receptionist_model->getReceptionistByIonUserIdFromUsers($data['receptionist']->ion_user_id)->permissions;
       $permissions = explode(',', $receptionist_permission);
       $option='<label for="exampleInputEmail1">'. lang('module_permission') .' </label><br>';
       foreach ($all_permissions as $all){
         if ( $all->feature == 'Patient' || $all->feature == 'Appointment' || $all->feature == 'Patient Encounter' || $all->feature == 'Finance' || $all->feature == 'Email') { 
