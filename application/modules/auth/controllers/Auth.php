...
class Auth extends MX_Controller {
	...
	function resend() {
		...
		// $dataSms = ['phone' => '9750182115', 'text' => 'Hello, CI'];
		// //$message = $this->sendSMS($dataSms);
		// // echo "<pre>"; print_r($message); exit;
		// // echo $message->sid;
		// $updateOTP = $this->otpsms_model->updateOTP($dataOtp, 0);
		// $this->_render_page('auth/otp');
		$checkPhone = $this->otpsms_model->checkPhoneNumber($_SESSION['userinput']);
		if($checkPhone) {
			//echo "<pre>"; print_r($checkPhone); exit;
			$number = '+91'.$_SESSION['userinput'];
			$dataOtp = rand ( 10000 , 99999 ); 
	...
	protected function sendSMS($data) {
		// Your Account SID and Auth Token from twilio.com/console
		  $sid = 'AC5e54b237ee9f2f220a0b0a9f50cdc6d2';
		  $token = '2f41c24b7377697e120ac3895993630c';
		  $client = new Client($sid, $token);
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if($_SESSION['login_methode'] == 'email') {
				...
				if($this->ion_auth->checkotp($this->input->post('identity'), $this->input->post('password'))) {
					$this->session->set_flashdata('message', $this->ion_auth->messages());
					redirect('home', 'refresh');
				} else {
					$data['message'] = '<p> Invalid OTP. </p>';
					...
					$this->_render_page('auth/otp', $data);
				}
			}
		}
		else
	...
	{
		$data['title'] = "Logout";
		$updateOTP = $this->otpsms_model->updateOTP(0,0);
		//log the user out
	...
	function create_user()
	{
		...
		if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $this->input->post('phone'), $additional_data))
		{
