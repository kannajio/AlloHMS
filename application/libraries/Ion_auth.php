...
class Ion_auth {
     ...
     * @author Mathew
     * */
    public function register($username, $password, $email, $phone, $dfg, $additional_data = array(), $group_ids = array()) { //need to test email activation
        $this->ion_auth_model->trigger_events('pre_account_creation');

        $email_activation = $this->config->item('email_activation', 'ion_auth');

        if (!$email_activation) {
            $id = $this->ion_auth_model->register($username, $password, $email, $phone, $dfg, $additional_data, $group_ids);
            if ($id !== FALSE) {
    ...
    public function in_group($check_group, $id = false, $check_all = false) {
        ...
        foreach ($check_group as $key => $value) {
             ...
             * if all, !in_array
             */
            if (in_array($value, $groups) or $check_all) {
                /**
                 * if !all (default), true
                 * if all, false
                 */
