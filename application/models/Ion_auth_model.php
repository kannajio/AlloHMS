...
class Ion_auth_model extends CI_Model
{
	...
	public function updateSerialId($table,$userid,$hosp_id) {
		$query = $this->db->select('name')
		->where('id', $hosp_id)
		->get('hospital');
		$user = $query->row();

		$serialId = strtoupper(substr($user->name,0,2)).(1000 + $userid);
		$data = array();
		if($table == 'payroll') {
		...
		$this->db->update($table, $data, array('ion_user_id' => $userid));
		return true;
	}

	public function checkotp($identity, $password)
	{
